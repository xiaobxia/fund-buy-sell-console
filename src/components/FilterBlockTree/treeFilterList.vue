<template>
  <div class="tree-filter-list">
    ...
    <div v-if="type === 'single'" :style="{'max-height': listHeight}" class="single-content infinite-content">
      ...
      <p v-if="loading" style="text-align: center; color: #bbb; padding: 5px 0">加载中...</p>
      <p v-if="noMore && !loading" style="text-align: center; color: #bbb; padding: 5px 0">无更多数据</p>
    </div>
    <!--    多选为复选框组-->
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      return this.$http.post('ucenterGh/org/getOrgListBySearchName', {
